...
In Year 5, our group made significant progress in advancing NAS algorithms through two primary contributions: the design and development of VINARCH, an interactive visual analytics tool for NN archeology, and the implementation of incremental checkpointing methods for efficient storage in evolutionary NAS algorithms. Kin NG, a postdoctoral researcher at UTK, joined the project.

We developed VINARCH, a tool designed to explore and analyze the evolution of neural network architectures generated by NAS algorithms. VINARCH enables researchers to identify and visualize matching subsequences in the structure of neural networks, providing deeper insights into architectural patterns and their relationship to the performance of specific models. We validated the tool's robustness and usability through rigorous testing on neural networks trained with datasets for classifying protein diffraction conformations. By providing an intuitive platform for investigating NN evolution, VINARCH addresses a critical need for insights into how architectures evolve throughout the NAS evolutionary process.

Second, we assessed the effectiveness of incremental checkpointing strategies within the context of NAS algorithms, specifically evaluating two approaches: the basic and list-based methods. The two algorithms optimize the storage of neural network weights and biases across checkpoints while maintaining prediction fidelity. We conducted extensive benchmarking to compare the storage requirements and accuracy retention of full, basic, and list checkpointing approaches. We analyzed different error threshold values to determine the optimal balance between storage efficiency and accuracy. Our preliminary results demonstrated storage savings of up to 38% while maintaining over 99% prediction fidelity compared to full checkpointing. These findings laid the groundwork for future research into advanced checkpointing strategies aimed at enhancing the storage efficiency of NN models without compromising their performance.

This year, we initiated a collaboration with Tom Peterka (ANL) and Orcun Yildiz (ANL). We are working on a prototype to enhance our A4NN workflow, originally developed in Year 4, by integrating a more advanced workflow orchestrator, Wilkins. This enhanced A4NN workflow also extends the decoupled tasks to not only enable early stopping of neural network training but also identify redundant networks based on structural similarity measures. Wilkins simplifies the execution of multiple decoupled tasks and efficiently manages cyclic dependencies between them. We tested this workflow prototype on baseline datasets. Preliminary results show that Wilkins incurs minimal overhead while offering greater flexibility for managing multiple tasks. Moreover, the extended task of identifying redundant NNs provides substantial savings on top of the previous approach.

In Year 6 of this project, we will extend our VINARCH tool by incorporating additional similarity measures beyond text alignment, such as genetic alignment, graph kernel distances, and network alignment metrics. For incremental checkpointing, we plan to explore advanced strategies, including Merkle Tree approaches, to optimize storage. We will also leverage metadata generated by incremental checkpointing to study training patterns, such as convergence behaviors and weight evolution, and their relationship to model performance. For the A4NN workflow, we will perform more rigorous testing with Wilkins to evaluate its performance across other datasets and scenarios. We will also integrate the incremental checkpointing techniques into the workflow as an additional task.

## Results for 2025/2026
In Year 6, we advanced our Analytics for Neural Networks (A4NN) workflow to address the growing computational and energy challenges of neural architecture search (NAS). The new version, A4NN.2, targets inefficiencies caused by redundant training of similar neural networks and limited workflow flexibility. Building on prior work that reduced training time through early termination of underperforming models, A4NN.2 introduces mechanisms to further reduce unnecessary computation while preserving model accuracy. 

A4NN.2 integrates two key enhancements. First, it incorporates a structural similarity engine that identifies and eliminates redundant neural network architectures during the NAS process, reducing repeated training of structurally similar models. Second, the workflow adopts a modular orchestration approach that decouples NAS algorithms from analytics tasks, improving flexibility and enabling easier integration of new components. Together, these enhancements reduce overall computational cost, measured through proxies such as floating-point operations (FLOPs) and training time, which directly relate to energy consumption. 

We evaluated A4NN.2 on standard image classification benchmarks (CIFAR10 and CIFAR100) as well as on protein diffraction datasets generated from X-ray Free Electron Laser experiments. Across these benchmarks, A4NN.2 achieved substantial efficiency gains while maintaining accuracy, with reductions of up to 64.3% in FLOPs and 59% in training time for CIFAR10, and up to 53.4% in FLOPs and 45% in training time for CIFAR100. On protein diffraction datasets, the workflow reduced FLOPs by 47.4% to 53.5% across varying experimental conditions while preserving accurate model selection. To support reproducibility and community use, we released workflow configurations, task implementations, and training metadata for transparent validation and reuse.

## Visits and meetings
Michela Taufer and Kin NG attended the 2025 JLESC workshop in Argonne National Laboratory. 
Michela Taufer attended the 2024 JLESC workshop in Kobe Japan. 
Taufer will meet Tama and Miyashita during her visit at RIKEN on Dec 6-9 for the DOE/MEXT and R-CCS meeting.
...
* Talk: Georgia Channing, Ria Patel, Paula Olaya, Ariel Keller Rorabaugh, Silvina Caino-Lores, Osamu Miyashita, Catherine Schuman, Florence Tama, and Michela Taufer. 16th Joint Laboratory for Extreme Scale Computing (JLESC) Workshop, 2024. Talk: Composable Workflow for Accelerating Neural Architecture Search Using In Situ Analytics
* Short Paper: Seoyoung An, Georgia Channing, Catherine Schuman, and Michela Taufer, “VINARCH: A Visual Analytics Interactive Tool for Neural Network Archaeology” at IEEE Cluster 2023 in Santa Fe, New Mexico, 2023 {% cite AnEtAl2023 --file jlesc.bib %}. An also presented this work at the University of Tennessee Exhibition of Undergraduate Research and Creative Achievement (EUReCA), where she won an award for her poster presentation.  
