...
* One journal publication presenting the new ChASE library to be submitted in June/July 2019
* One conference publication presenting results related to the use of ChASE to solve very 
large eigenvalue problems generated by the BSE Jena code to be submitted in April 2019
...
the ChASE library into the Jena BSE code and test scalability and efficiency of the ChASE library
on Blue Waters when used to solve eigenproblems of increasing size and complexity. 
We are currently in the process of testing the performance of ChASE for the diagonalization of Bethe-Salpeter matrices for different physical problems related to exciton binding energies in ZnO and In2O3. We are performing strong and weak scaling tests and we also planning to explore the maximum rank of a matrix that we can now work with, using the new code. These results will allow us to precisely quantify the performance improvements and also to determine what new problems the modified code can address, that the old code could not, due to computational limitations. Our team is currently collecting data and preparing a scientific publication of these results.
