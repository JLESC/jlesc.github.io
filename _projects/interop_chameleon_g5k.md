---
layout: page_project
title: Toward Interoperability between Chameleon and Grid'5000 experimentation plateforms
date: 2017-06-07
updated: 
navbar: Research
subnavbar: Projects
...
Grid'5000 (Inria) experimentation plateforms.
...
perform cross-platfom deployment.
...
When a user converts a Grid'5000 image to a Chameleon image, the image format is first converted from Grid'5000 to Chamelon ones. Then the configuration needed to run on Chameleon infrastructure is added (e.g., cloud-init is installed with Chameleon configuration file). This adds virtual machine contextualization ability from Chameleon backend.

The second goal of the projet is to study Openstack deployment tool that would work over both Chameleon and Grid'5000.
...
This makes it easier for users to deploy the same environment on Grid'5000 and Chameleon testbeds, easing experiments involving both plateforms.
...
It is likely that Chameleon and Grid'5000 will converge to the same solution, reducing maintenance costs for both plateforms and easing cross experimentation for their users
